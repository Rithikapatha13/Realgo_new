...
export default function CompanyUsersReport() {
    ...
    const { data, isLoading } = useReportCompanyLinkedUsers(filters);
    ...
    const handleExport = () => {
        if (data?.items) {
            exportToExcel(data.items, "Company_Users_Report.xlsx");
        }
    };

    const handlePrint = () => {
        if (data?.items) {
            printHTMLTable(data.items, "Company Linked Users Report");
        }
    };
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm whitespace-nowrap">
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                            {isLoading ? (
                                <tr>
                                    <td colSpan="6" className="px-6 py-8 text-center text-slate-500">Loading records...</td>
                                </tr>
                            ) : data?.items?.length > 0 ? (
