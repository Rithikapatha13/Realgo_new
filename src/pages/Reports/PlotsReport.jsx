...
export default function PlotsReport() {
    ...
    const { data, isLoading } = useReportPlots(filters);
    const { data: projectsData } = useGetAllProjects(); // Assuming useGetAllProjects exists and returns { items: [...] }
    ...
    const handleExport = () => {
        if (data?.items) {
            exportToExcel(data.items, "Plots_Report.xlsx");
        }
    };

    const handlePrint = () => {
        if (data?.items) {
            printHTMLTable(data.items, "Plots Report");
        }
    };
                            ...
                            {isLoading ? (
                                ...
                                data.items.map((plot, idx) => (
                                    <tr key={idx} className="hover:bg-slate-50">
                                        <td className="px-6 py-4 font-medium text-slate-900">{plot.name}</td>
                                        <td className="px-6 py-4 text-slate-500">{plot["plot number"]}</td>
                                        <td className="px-6 py-4 text-slate-500">{plot.facing || "-"}</td>
                                        <td className="px-6 py-4 text-slate-500">{plot.size}</td>
                                        <td className="px-6 py-4 text-slate-500">{plot["plot category"]}</td>
                                        <td className="px-6 py-4 text-slate-500">{plot.associate_name || "-"}</td>
                                        <td className="px-6 py-4 text-slate-500">{plot.team_head_name || "-"}</td>
                                        <td className="px-6 py-4 text-slate-500">{plot.customer_name || "-"}</td>
